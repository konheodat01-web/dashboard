...
function wstApplyModeUI(){
  ...
  var tSeo = document.getElementById('wstToolsSeo');
  var tCon = document.getElementById('wstToolsContent');
  if (tSeo) tSeo.style.display = isC ? 'none' : 'flex';
  if (tCon) tCon.style.display = isC ? 'flex' : 'none';
  ['seo','content'].forEach(function(k){
...
// => chuyển chế độ bảng không xê dịch. Tổng luôn = 100%.
...
function wstUpdateSelInfo(){
  ...
  var cb = document.getElementById('wstPostsCopyBtn');
  if (cb) cb.innerHTML = n ? ('📋 Copy ' + n + ' URL') : '📋 Copy URL đã chọn';
}

// Copy URL các bài ĐÃ CHỌN trong popup danh sách bài viết (không chọn gì -> copy toàn bộ đang hiển thị).
function wstCopySelectedPostUrls(){
  var selKeys = Object.keys(_wstPosts.sel);
  var urls = (selKeys.length
    ? _wstPosts.items.filter(function(p){ return _wstPosts.sel[_wstUrlKey(p.link)]; })
    : wstFilteredPosts()
  ).map(function(p){ return p.link; }).filter(Boolean);
  if (!urls.length){ if (typeof toast === 'function') toast('Không có URL nào để copy', '#e74c3c'); return; }
  var text = urls.join('\n');
  var done = function(){ if (typeof toast === 'function') toast('📋 Đã copy ' + urls.length + ' URL', '#3fb950'); };
  var fallback = function(){
    try {
      ...
      if (ok) done(); else if (typeof toast === 'function') toast('❌ Không copy được', '#e74c3c');
    } catch(e){ if (typeof toast === 'function') toast('❌ Không copy được', '#e74c3c'); }
  };
...
function renderWsTrack(){
  ...
  if(!tbody) return;
  if (typeof wstApplyModeUI === 'function') wstApplyModeUI();
  if (typeof wstApplyColgroup === 'function') wstApplyColgroup();
  ...
  if(thead){
    thead.innerHTML = `<tr style="background:#f8f9fa;border-bottom:2px solid var(--gray-border)">
      <th style="padding:8px 6px;text-align:center"><input type="checkbox" id="wstSelectAll" onchange="wstToggleSelectAll(this)" style="cursor:pointer;accent-color:var(--red)"></th>
      <th onclick="wstHandleSort('url301')" style="padding:8px 10px;text-align:left;font-size:11px;cursor:pointer;user-select:none">URL hiện tại (301) ${wstGetSortIndicator('url301')}</th>
    ...
    <div style="display:flex;flex-direction:column;gap:12px">
      ...
      <div class="form-group">
        ...
        <select id="wst_indexed" style="width:100%;padding:6px;border-radius:6px;background:#2d3748;color:#fff;border:1px solid #4a5568">
          ...
          <option value="Cần kiểm tra">⚠️ Cần kiểm tra</option>
...
function openWebsiteForm(id=null){
  ...
  if (typeof _currentFormTags !== 'undefined') {
    _currentFormTags = (w && Array.isArray(w.tags)) ? [...w.tags] : [];
    if (typeof renderCurrentTags === 'function') renderCurrentTags();
  }
...
function wf301SelectSource(wsId){
  ...
  if (typeof _currentFormTags !== 'undefined') {
    _currentFormTags = (src && Array.isArray(src.tags)) ? [...src.tags] : [];
    if (typeof renderCurrentTags === 'function') renderCurrentTags();
  }
...
function saveWebsite(){
  ...
  const obj={
    ...
    tags: (typeof _currentFormTags !== 'undefined') ? [..._currentFormTags] : [],
    team,
    owner: 'Công ty',
    group: '',
    is301,
  ...
  if(!found){
    ...
    body.innerHTML=`
      <div style="padding:12px 16px;background:#f0faf4;border-bottom:1px solid #b7e4c7;display:flex;align-items:center;gap:8px">
        ...
        <span style="font-weight:600;color:#27ae60">Đã nhận diện: ${wstBrandHtml(w)}</span>
      </div>
  ...
  if (token) {
    try {
      ...
      if (inspectResult) {
        ...
        saveWsTrack(wsId);
        wstAddChangelog(wsId, 'manual_entry', `Kiểm tra Index bằng GSC Inspection: ${finalStatus === 'Đã index' ? 'Đã Index ✅' : 'Chưa Index ❌'}`);
        renderWsTrack();
        toast(`✓ GSC Inspect: ${finalStatus === 'Đã index' ? 'Đã Index' : 'Chưa Index'}`, '#27ae60');
        return;
      }
    } catch (gscErr) {
...
async function wstCheckIndexStatusSerper(wsId) {
  ...
  try {
    ...
    const finalStatus = hasResults ? 'Đã index' : 'Chưa index';
    ...
    saveWsTrack(wsId);
    wstAddChangelog(wsId, 'manual_entry', `Kiểm tra Index bằng Serper: ${finalStatus === 'Đã index' ? 'Đã Index ✅' : 'Chưa Index ❌'}`);
    
    renderWsTrack();
    toast(`✓ Check Index: ${finalStatus === 'Đã index' ? 'Đã Index' : 'Chưa Index'}`, '#27ae60');
  } catch (err) {
...
// --- TAGS LOGIC ---
function initTagsUI() {
  const input = document.getElementById('wf_tag_input');
  if(!input) return;
  ...
  document.addEventListener('click', (e) => {
    const sugg = document.getElementById('wf_tag_suggestions');
    const container = document.getElementById('wf_tags_container');
...
function handleTagInput() {
  const val = document.getElementById('wf_tag_input').value.trim().toLowerCase();
  const sugg = document.getElementById('wf_tag_suggestions');
  const available = getAllAvailableTags();
  
  let matches = available.filter(t => t.toLowerCase().includes(val) && !_currentFormTags.includes(t));
  
  let html = '';
  matches.forEach(t => {
    html += `<div class="tag-sugg-item" onclick="addTag('${t.replace(/'/g, "\\'")}')" onmouseover="this.style.background='#30363d'" onmouseout="this.style.background='transparent'" style="padding:6px 10px;cursor:pointer;color:#c9d1d9;border-bottom:1px solid #30363d;transition:background 0.2s;">${t}</div>`;
  });
  
  const rawVal = document.getElementById('wf_tag_input').value.trim();
  if(rawVal && !available.find(t => t.toLowerCase() === rawVal.toLowerCase()) && !_currentFormTags.find(t => t.toLowerCase() === rawVal.toLowerCase())) {
    html += `<div class="tag-sugg-item" onclick="addTag('${rawVal.replace(/'/g, "\\'")}')" onmouseover="this.style.background='#30363d'" onmouseout="this.style.background='transparent'" style="padding:6px 10px;cursor:pointer;color:#58a6ff;transition:background 0.2s;">+ Tạo tag: <b>${rawVal}</b></div>`;
  }
...
function addTag(tag) {
  if(!_currentFormTags.includes(tag)) {
    _currentFormTags.push(tag);
    if (typeof renderCurrentTags === 'function') renderCurrentTags();
  }
  document.getElementById('wf_tag_input').value = '';
  document.getElementById('wf_tag_input').focus();
  document.getElementById('wf_tag_suggestions').style.display = 'none';
}

function removeTag(tag) {
  _currentFormTags = _currentFormTags.filter(t => t !== tag);
  if (typeof renderCurrentTags === 'function') renderCurrentTags();
}
...
function renderCurrentTags() {
  const container = document.getElementById('wf_tags_container');
  if(!container) return;
  const inputEl = document.getElementById('wf_tag_input');
  
  // Xóa các tag cũ (chỉ giữ lại input)
  Array.from(container.children).forEach(child => {
    if(child.id !== 'wf_tag_input') container.removeChild(child);
  });
  
  _currentFormTags.forEach(t => {
    ...
    pill.innerHTML = `<span>${t}</span><span style="cursor:pointer;font-weight:bold;font-size:14px;line-height:1;opacity:0.8;" onmouseover="this.style.opacity='1'" onmouseout="this.style.opacity='0.8'" onclick="removeTag('${t.replace(/'/g, "\\'")}')">&times;</span>`;
